...
<div class="md:mt-1 justify-center items-center relative h-fit">

    <div class="flex flex-col w-full max-w-[350px] mx-auto font-work text-lg">
        <div class="grid grid-cols-2 gap-x-4 gap-y-1 items-center max-w-md">
            <label for="vacancy" class="font-medium">Vacancy Rate (%):</label>
            <input type="number" id="vacancy" value="10" min="0" max="100" class="w-16 py-0 px-2"></input>
            <label for="threshold" class="font-medium">Threshold (%):</label>
            <input type="number" id="threshold" value="50" min="0" max="100" class="w-16 py-0 px-2"></input>
        </div>

        <div class="flex justify-start gap-x-4 my-2">
            <button class="bg-cyan-800 hover:bg-cyan-900 text-white font-bold py-[0.5] px-4 rounded" id="start-button">Start </button>
            <button class="bg-cyan-800 hover:bg-cyan-900 text-white font-bold py-[0.5] px-4 rounded" id="reset-button">Reset </button>
        </div>

        <div id="segregation_chart" class="w-full my-0" style="height: 350px;"></div>

        <div class="grid grid-row-2 mt-1 gap-x-4 gap-y-1 items-center max-w-md">
            <div class="">Unhappiness Index: <span id="unhappiness-value" class="">0.0</span>%s</div>
            <div class="">Dissimilarity Index: <span id="dissimilarity-value" class="">0.00</span> </div>
        </div>
</div>
</div>
